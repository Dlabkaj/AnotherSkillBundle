...
5. **Ingest — atomic unit. Never mark `done` unless every step below completed** (AI judgment):
   ...
   - **Local-backend branch** — if `task.md` has `INGEST_BACKEND: local:<model>`: do NOT extract inline. Instead invoke [LocalModelIngestionSkill](../LocalModelIngestionSkill/SKILL.md) — compose the extraction prompt (this source's `RESEARCH_FOCUS`, language, cite using the literal URL string not the word "SOURCE_URL", "tag crisp atoms `*(unverified)*`") and run `{{SKILLS_ROOT}}/LocalModelIngestionSkill/Run-LocalIngestion.ps1 -RawFile <next_candidate.raw> -WikiTarget <target sub-page> -Prompt "<composed>" -Model <model>` (orchestrated mode: the runner injects the exact script path into the prompt — use that). Read its `=== LocalIngestion status ===` block; use `CHARS_APPENDED` as `chars_added_this_scrape`. If `RESULT: FAILED` → `mark <task_dir> <url> skipped-ingest` → step 6. Otherwise skip the inline "Extract facts" bullet below and continue to the measure/update/mark steps. Claude still owns every state write.
